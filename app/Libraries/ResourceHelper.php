...
class ResourceHelper
{
	 ...
	 * @return array                
	 */
	public static function showAllResource($module = '', $model_name = '', $page_size = 0, $page = 0, $sorting = [], $relationship = [])
	{
		$source = "\App\Modules\\".$module."\Models\\".$model_name;
        if (empty($sorting)) {
            $model = $source::with($relationship)->get();
        } else {
            $model = $source::with($relationship)->orderBy($sorting['field'], $sorting['sort_order'])->get();
        }
		$pagination = self::validatePagination($page_size, $page);
		if ($pagination == true) {
			// $model = $source::skip($start)->take($limit)->get();
			$model = $source::paginate($page_size);
			if (count($relationship)) {
				// $model = $source::with($relationship)->take($limit)->get();
				$model = $source::with($relationship)->paginate($page_size);
			}
		}
		if (count($model) > 0) {
			$response = [
	 ...
	 * @return array
	 */
	public static function forceBatchDeleteResource($module = '', $model_name = '', $resource_id = 0, $resource_field = '')
	{
		$source = "\App\Modules\\".$module."\Models\\".$model_name;
		if ($model = $source::withTrashed()->whereIn($resource_field, $resource_id)) {
			if ($model->forceDelete()) {
                $response = [
                    'code'      => '200',
                    'status'    => StatusHelper::getSuccessResponseStatus(),
        ...
        /**
	 * Batch update resources in the model within the module using the request parameters as fields and values
	 * @param  Reqeust $request_instance
	 * @param  string $table
